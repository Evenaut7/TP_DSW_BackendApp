...
export class LocalidadController {
  findAll = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ message: 'Error fetching localidades', error: error.message });
    }
  };
  findOne = async (req: Request, res: Response) => {
    try {
      const id = Number.parseInt(req.params.id);
      const localidad = await orm.em.findOneOrFail(
        Localidad,
        { id },
  ...
  add = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ message: 'Error adding localidad', error: error.message });
    }
  };
