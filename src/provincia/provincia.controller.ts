...
export class ProvinciaController {
  ...
  add = async (req: Request, res: Response) => {
    try {
      const existe = await em.findOne(Provincia, { nombre: req.body.nombre });
      if (existe) {
        res
          .status(409)
          .json({ message: `Ya existe una provincia con el nombre '${req.body.nombre}'` });
        return;
      }
      const newProv = em.create(Provincia, req.body);
      await em.flush();
      res.status(201).json({ message: 'Provincia added successfully', data: newProv });
    } catch (error: any) {
  ...
  update = async (req: Request, res: Response) => {
    try {
      ...
      await em.flush();
      res.status(200).json({ message: 'Provincia updated successfully', data: provincia });
    } catch (error: any) {
