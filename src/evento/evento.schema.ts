...
export const eventoSchema = z.object({
  body: z.object({
    ...
    horaDesde: z
      .string()
      .nonempty({ message: 'La hora inicial no puede estar vacia' })
      .refine((date) => !isNaN(Date.parse(date)), { message: 'Formato de fecha Invalido' }),
    horaHasta: z
      .string()
      .nonempty({ message: 'La hora final no puede estar vacia' })
      .refine((date) => !isNaN(Date.parse(date)), { message: 'Formato de fecha Invalido' }),
    ...
    tags: z
      .array(z.number().positive({ message: 'El id del tag debe ser un numero positivo' }))
      .optional(),
  }),
});
