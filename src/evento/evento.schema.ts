...
export const eventoSchema = z.object({
  body: z.object({
    ...
    horaDesde: z.string().nonempty("La hora inicial no puede estar vacia").refine((date) => !isNaN(Date.parse(date)), { message: "Formato de fecha Invalido" }),
    horaHasta: z.string().nonempty("La hora final no puede estar vacia").refine((date) => !isNaN(Date.parse(date)), { message: "Formato de fecha Invalido" }),
