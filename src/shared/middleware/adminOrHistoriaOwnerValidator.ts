...
export async function adminOrHistoriaOwnerValidator(
  req: Request,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    const historia = await em.findOneOrFail(
      Historia,
      { id },
    ...
    res
      .status(403)
      .json({ message: 'No autorizado: debe ser dueño del PDI de la historia o admin' });
  } catch (error: any) {
